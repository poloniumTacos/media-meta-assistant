...
```bash
git clone https://github.com/poloniumTacos/media-meta-assistant.git
```

2. Navigate to the cloned directory:

```bash
cd media-meta-assistant
```
...
Before using Media Meta Assistant, you need to rename `config_example.json` to `config.json` and provide your TMDb API key in a `config.json` file. Follow these steps:
...
2. Run the Media Meta Assistant script by executing the following command:
